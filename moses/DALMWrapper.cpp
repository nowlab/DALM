...
public:
  ...
  */
  ...
  /*
  virtual int Compare(const FFState& other) const {
    int ret = CompareA(other);
    std::cerr << "Compare=" << ret << std::endl;
    return ret;
  }
  */
};
...
void LanguageModelDALM::CalcScore(const Phrase &phrase, float &fullScore, float &ngramScore, size_t &oovCount) const
{
  ...
  //std::cerr << "before_boundary=" << prefixScore << " fullScore=" << fullScore << " ngramScore=" << ngramScore << " oovCount=" << oovCount << std::endl;
...
FFState *LanguageModelDALM::EvaluateWhenApplied(const ChartHypothesis& hypo, int featureID, ScoreComponentCollection *out) const
{
  ...
  //std::cerr << hypo.ToString() << std::endl;
  ...
  if(hypoSize > 0) {
    ...
    if(word.GetFactor(m_factorType) == m_beginSentenceFactor) {
      ...
      /*
      std::cerr << "X[ ";
      for(std::size_t i = 0; i < prevHypo->GetCurrTargetPhrase().GetSize(); ++i){
        std::cerr << prevHypo->GetCurrTargetPhrase().GetWord(i).GetString(m_factorType).as_string() << " ";
      }
      std::cerr << "] ";// << prevHypo->ToString() << " ";
      */
      ...
      //std::cerr << "@" << UntransformLMScore(prevHypo->GetScoreBreakdown().GetScoresForProducer(this)[0]) << std::endl;
  ...
  //std::cerr << " score=" << hypoScore;
  hypoScore = TransformLMScore(hypoScore);
  hypoScore -= hypo.GetTranslationOption().GetScores().GetScoresForProducer(this)[0];
  //std::cerr << " left=" << (int) newState->GetPrefixLength() << " right=" << (int) newState->GetRightContext().get_count() << " score=" << hypoScore << std::endl;
  ...
  unsigned char &prefixLength) const
{
  ...
  if (newState->LargeEnough()) {
    ...
    /*
    float dbg_score = m_lm->query(wid, s);
    if(std::abs(((score>0)?-score:score)-dbg_score) > 0.001){
      std::cerr << "A score=" << score << " answer=" << dbg_score << std::endl;
    }
    */
    //std::cerr << score << "c ";
...
) const
{
  ...
  for(size_t prefixPos = 0; prefixPos < prevPrefixLength; prefixPos++) {
    ...
    //std::cerr << f.status.word << ":";

    if (newState->LargeEnough()) {
      float score = m_lm->query(f, state, gap);
      /*
      float dbg_score = m_lm->query(f.status.word, s1) - m_lm->query(f.status.word, s2);
      if(std::abs(score-dbg_score) > 0.001){
        std::cerr << "B score=" << score << " answer=" << dbg_score << std::endl;
      }
      */
      //std::cerr << score << "b ";
      hypoScore += score;
      ...
      float dbg_score = m_lm->query(f.status.word, s1) - m_lm->query(f.status.word, s2);
      if(std::abs(score-dbg_score) > 0.001){
        std::cerr << "C score=" << score << " answer=" << dbg_score << std::endl;
      }
      */
      //std::cerr << score << "a ";
      hypoScore += score;
