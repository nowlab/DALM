...
void BstDA::make_da(std::string &pathtotreefile, ValueArrayIndex *, Vocabulary &vocab) {
    ...
    for(std::size_t i = 0; i < n_entries; ++i){
        if((i+1) % ten_percent == 0){
            logger_ << "BstDA[" << daid_ << "] " << (i+1)/ten_percent << "0% done. " << sw.sec() << " sec. " << loop_counts_ << " s checked, " << skip_counts_ << " s skipped." << Logger::endi;
        }
    ...
    logger_ << "BstDA[" << daid_ << "] " << "Total construction time is " << sw.sec() << " seconds." << Logger::endi;
