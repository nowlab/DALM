#ifndef REVERSEFILE_H_
#define REVERSEFILE_H_
...
namespace DALM {
	...
	class ReverseFile {
		public:
			ReverseFile(std::string reversefile, Vocabulary &vocab) : vocab_(vocab), file_(reversefile){
				read_header();
			}
			...
			void get_ngram(unsigned short &order, VocabId *&ngram, float &prob, float &bow){
				std::string word;
				...
				if(sep == '\t'){
					...
					prob = (float)std::atof(value_str.c_str());
				}else{
					prob = 0.0;
				}
				...
				if(sep == '\t'){
					...
					bow = (float)std::atof(value_str.c_str());
				}else{
					bow = 0.0;
				}
			}
